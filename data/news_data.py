...
class FinnhubNewsClient:
    def __init__(self, api_key:str):
        self.api_key=api_key; self.base_url='https://finnhub.io/api/v1'; self.session: Optional[aiohttp.ClientSession]=None; self.timeout=aiohttp.ClientTimeout(total=15)
    ...
    async def fetch_market_news(self, category='general') -> list[dict]:
        if not self._ready(): raise RuntimeError('Finnhub session not connected')
    ...
    async def fetch_ticker_news(self, symbol:str, start_date:str, end_date:str) -> list[dict]:
        if not self._ready(): raise RuntimeError('Finnhub session not connected')
