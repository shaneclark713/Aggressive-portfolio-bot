...
from telegram_bot.ui_helpers import authorize_update, get_ui_settings
...
def build_analytics_handlers(app_services: dict, config_service, admin_chat_id: int):
    ...
    def _option_chain_rows() -> list[dict]:
        value = get_ui_settings(settings_repo, "last_option_chain", {"rows": []})
        return list(value.get("rows", []))

    def _options_flow_rows() -> list[dict]:
        value = get_ui_settings(settings_repo, "options_flow_rows", {"rows": []})
        return list(value.get("rows", []))

    async def ml_weights(update, context):
        if not await authorize_update(update, admin_chat_id):
            return
        weights = get_ui_settings(settings_repo, "ml_weights", {})
