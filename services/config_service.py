...
class ConfigService:
    ...
    def _validate_filter_value(self, category: str, field: str, value: Any) -> None:
        if field.endswith("_min") or field.endswith("_max") or field.endswith("_pct"):
        ...
        if field in {"contract_min_price", "price_min"} and isinstance(value, (int, float)) and value < 0:
            raise ValueError(f"{field} must be non-negative")
        if field == "time_of_day_restrictor" and value:
            parts = str(value).split(":")
            if len(parts) != 2 or not all(part.isdigit() for part in parts):
                raise ValueError("time_of_day_restrictor must look like HH:MM")
