...
class InstitutionalAIEcosystemEngine:
    """Institutional AI ecosystem coordinator with consensus + risk governance."""
    ...
    def build(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        ecosystem_score = 50
        ecosystem_score += int((int(consensus.get("consensus_score",50))-50)*0.30)
        ecosystem_score += int((int(review.get("review_score",50))-50)*0.25)
        ecosystem_score += int((int(memory.get("autonomy_win_rate",50))-50)*0.15)
        ecosystem_score += int((int(flow.get("expansion_quality_score",50))-50)*0.15)
