...
class Spy0DteService:
    ...
    US_EVENT_KEYWORDS=("ppi","cpi","pce","fed","fomc","powell","claims","payroll","nfp")

    def __init__(self,telegram_app,chat_id:int,market_client,news_client,econ_client,tradier_client=None):
        self.telegram_app=telegram_app; self.chat_id=chat_id; self.market_client=market_client
        self.news_client=news_client; self.econ_client=econ_client; self.tradier_client=tradier_client
        self.market_tz=ZoneInfo('America/New_York')
        self.dealer_gamma=DealerGammaService(); self.cross_market=CrossMarketIntelligenceService(market_client)
        self.narrative_engine=MarketNarrativeEngine(); self.playbook_engine=TacticalPlaybookEngine()
        self.probability_engine=ProbabilityMatrixEngine(); self.execution_timing_engine=ExecutionTimingEngine()
        self.adaptive_exit_engine=AdaptiveExitEngine(); self.autonomous_scaling_engine=AutonomousScalingEngine()
        self.session_personality_engine=SessionPersonalityEngine(); self.trap_detection_engine=TrapDetectionEngine()
    ...
    async def analyze(self)->dict[str,Any]:
        ...
        latest=float(minute_df['close'].iloc[-1]) if not minute_df.empty else 0.0
        vwap=latest; rsi_5m=50.0
        dealer_gamma={}; structure={'score':0,'bias':'balanced / tactical'}
        cross_market=await self.cross_market.analyze(); narrative={}
        ...
        return {'latest':latest,'vwap':vwap,'cross_market':cross_market,'probabilities':probabilities,'execution_timing':execution_timing,'adaptive_exits':adaptive_exits,'autonomous_scaling':autonomous_scaling,'session_personality':session_personality,'trap_detection':trap_detection,'trade_memory':trade_memory,'theta_protection':theta_protection,'institutional_flow':institutional_flow,'ai_review':ai_review,'autonomous_mutation':autonomous_mutation}
