...
def build_handlers(app_services, config_service, admin_chat_id: int):
    ...
    options_chain_ingest = app_services.get("options_chain_ingest_service") or OptionsChainIngestService(
        settings_repo,
        app_services.get("tradier_client"),
    )
    ...
    def _profile_from_scan(scan_type: str) -> str:
        return SCAN_TYPE_TO_PROFILE.get(
            scan_type,
            scan_type if scan_type in SCAN_TYPE_TO_PROFILE.values() else "overall",
        )
    ...
    async def _snapshot_status_text(context, profile: str | None = None) -> str:
        ...
        return format_snapshot_status(profile, await discovery.snapshot_status(profile))

    async def _passers_text(context, scan_type: str | None = None, limit: int = 12) -> str:
        scan_type = (scan_type or _active_scan_type(context)).lower()
        if discovery is None:
    ...
    async def _set_ml_weight(update, context):
        ...
        if len(context.args) != 2:
            await update.message.reply_text("Usage: /set_ml_weight <key> <value>")
            return
        weights = _get_ml_weights()
        weights[context.args[0]] = _coerce_scalar(context.args[1])
    ...
    async def _sync_positions(update, context):
        if not await _authorize_update(update):
            return
        sync_live = getattr(position_sync_service, "sync_live_positions", None)
        rows = await sync_live() if callable(sync_live) else {}
    ...
    async def _profile_exec_status(update, context):
        ...
        await update.message.reply_text(
            format_profile_execution_status(
                normalized_mode,
                profile_store.display_strategy(strategy),
                profile,
            ),
            parse_mode="HTML",
        )
    ...
    async def _submit_ladder(update, context):
        ...
        if live_execution_service is None:
            await update.message.reply_text("Live execution service unavailable.")
            return
    ...
    async def _guarded_callback(update, context):
        ...
        if data.startswith("scan|"):
            ...
            if action == "status":
                await query.edit_message_text(
                    await _scan_status_text(context),
                    parse_mode="HTML",
        ...
        if data.startswith("exec|"):
            ...
            if action == "ladder":
                if live_execution_service is None:
                    await query.edit_message_text("Live execution service unavailable.", reply_markup=build_execution_menu_keyboard())
                    return
                ...
                return
            if action in {"trailing", "open_trails"}:
                await query.edit_message_text(
            ...
            if action == "submit_ladder":
                if live_execution_service is None:
                    await query.edit_message_text("Live execution service unavailable.", reply_markup=build_execution_menu_keyboard())
                    return
                if hasattr(live_execution_service, "build_exit_ladder"):
                    ...
                    text = format_exit_ladder_submission(plan)
                else:
                    plan = await live_execution_service.submit_stock_ladder(
                        "SPY",
                        "LONG",
                    ...
                    text = format_ladder_submission(plan)
