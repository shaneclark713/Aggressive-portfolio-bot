...
class UniverseFilter:
    ...
    async def build_daily_watchlist(self) -> Dict[str, List[str]]:
        ...
        for symbol in CORE_EQUITIES:
            ...
            try:
                ...
                premarket = {"premarket_volume": 0, "premarket_gap_min_percent": 0.0}
            ...
            metrics["premarket_gap_min_percent"] = float(premarket.get("premarket_gap_min_percent", 0) or 0)
            ...
            metric_snapshot[symbol] = {
                ...
                "float_millions": round((metrics["max_float"] or 0) / 1_000_000, 2) if metrics["max_float"] else 0,
            }
            ...
            if self._passes_settings(metrics, DAY_TRADE_SETTINGS, descriptive_defaults):
                ...
                rejected[symbol] = self._describe_rejection(metrics, DAY_TRADE_SETTINGS, descriptive_defaults)
    ...
    def _passes_settings(self, metrics: Dict[str, Any], settings: Dict[str, Any], descriptive_defaults: Dict[str, Any]) -> bool:
        max_float_limit = descriptive_defaults.get("max_float")
        if max_float_limit and metrics.get("max_float") is not None and metrics["max_float"] > max_float_limit:
            return False
        if metrics.get("premarket_volume", 0) < descriptive_defaults.get("min_premarket_vol", 0):
            return False
        if metrics.get("premarket_gap_min_percent", 0) < descriptive_defaults.get("premarket_gap_min_percent", 0):
            return False
        return (
    ...
    def _describe_rejection(self, metrics: Dict[str, Any], settings: Dict[str, Any], descriptive_defaults: Dict[str, Any]) -> str:
        if descriptive_defaults.get("max_float") and metrics.get("max_float") is not None and metrics["max_float"] > descriptive_defaults["max_float"]:
            return "max_float"
        if metrics.get("premarket_volume", 0) < descriptive_defaults.get("min_premarket_vol", 0):
