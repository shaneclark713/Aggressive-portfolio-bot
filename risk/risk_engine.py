...
class RiskEngine:
    ...
    def calculate_trade_parameters(
        ...
        if any(v is None or v <= 0 for v in [entry_price, atr, recent_swing_high, recent_swing_low]):
            return {"is_valid": False, "reason": "Invalid numeric inputs"}

        if recent_swing_low >= recent_swing_high:
            return {"is_valid": False, "reason": "Invalid swing structure"}

        if side == "LONG":
            stop = recent_swing_low - (atr * self.atr_multiplier_sl)
            take = recent_swing_high
            risk = entry_price - stop
            reward = take - entry_price
        elif side == "SHORT":
            stop = recent_swing_high + (atr * self.atr_multiplier_sl)
            take = recent_swing_low
        ...
        return {
            ...
            "is_valid": valid,
            "reason": "Passed" if valid else f"Poor structural R:R ({rr:.2f})",
        }
