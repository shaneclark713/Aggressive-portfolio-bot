...
def build_handlers(app_services, config_service, admin_chat_id: int):
    ...
    async def _pending_text(update, context):
        ...
        try:
            ...
            await update.message.reply_text(f"Invalid value for {category}.{field}: {exc}\nSend a new value or /cancel.")
            return
        except Exception as exc:
            await update.message.reply_text(f"Could not update {category}.{field}: {exc}\nSend /cancel to exit.")
            return
        context.user_data.pop("pending_filter_edit", None)
    ...
    async def _guarded_callback(update, context):
        ...
        if data.startswith("set|preset|"):
            ...
            await query.edit_message_text(f"Preset updated to: {preset_name}", reply_markup=build_control_panel_keyboard())
            return
