...
class PolygonMarketDataClient:
    ...
    async def get_historical_data(self, symbol: str, multiplier: int = 1, timespan: str = "day", start_date: str | None = None, end_date: str | None = None) -> pd.DataFrame:
        ...
        params = {"adjusted": "true", "sort": "asc", "limit": self._default_limit(timespan), "apiKey": self.api_key}
    ...
    async def get_premarket_snapshot(self, symbol: str) -> dict:
        ...
        minute_df = await self.get_historical_data(symbol=symbol, multiplier=5, timespan="minute", start_date=session_date, end_date=session_date)
        if minute_df.empty:
            return {"premarket_volume": 0.0, "premarket_gap_min_percent": 0.0}
        ny_index = minute_df.index.tz_convert("America/New_York")
        premarket_mask = (((ny_index.hour > 4) | ((ny_index.hour == 4) & (ny_index.minute >= 0))) & ((ny_index.hour < 9) | ((ny_index.hour == 9) & (ny_index.minute < 30))))
        ...
        return {"premarket_volume": premarket_volume, "premarket_gap_min_percent": gap_pct}
