...
async def handle_control_panel_callback(update, context, config_service, settings_repo):
    ...
    if data == "cp|presets":
        await query.edit_message_text(
            "Select preset",
            reply_markup=preset_keyboard(FILTER_PRESETS),
        )
        return
    ...
    if data.startswith("cpreset|"):
        preset = data.split("|", 1)[1]
        config_service.set_active_preset(preset)
        await query.edit_message_text(f"Active preset changed to {preset}")
        return

    if data.startswith("cmode|"):
        mode = data.split("|", 1)[1]
        config_service.set_execution_mode(mode)
        await query.edit_message_text(f"Execution mode changed to {mode}")
