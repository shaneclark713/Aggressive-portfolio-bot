...
class PremarketService:
    ...
    async def run(self):
        ...
        try:
            watchlists = await self.watchlist_service.build_watchlists(scan_type="premarket")
        except Exception as exc:
            logger.exception("Premarket watchlist build failed: %s", exc)
            await self.telegram_app.bot.send_message(
                chat_id=self.chat_id,
                text=f"🌅 <b>5:30 AM Pre-Market Report</b>\n\n<b>Status:</b> Watchlist build failed.\n<b>Error:</b> {exc}",
                parse_mode="HTML",
            )
            return
        ...
        sections = {
            "Configuration": [
                ...
                f"Premarket preset: {self.config_service.get_profile_preset('premarket')}",
